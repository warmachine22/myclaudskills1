---
name: finviz-sentiment-premarket
description: Pre-market (9:20 AM ET, weekdays) sentiment + 3x ETF call — updates the live dashboard artifact and pushes to phone
---

PRE-MARKET market sentiment run (9:20 AM ET, ~10 min before the US open).
...
STEP 1 — prepare
  python ".claude\skills\finviz-market-sentiment\finviz_sentiment.py" prepare --limit 120 --worklist "<SENTIMENT_RUNS_DIR>\premarket-worklist.txt" --state "<SENTIMENT_RUNS_DIR>\premarket-state.json"

STEP 2 — score every headline yourself
Read <SENTIMENT_RUNS_DIR>\premarket-worklist.txt
Score EVERY id using the rubric in SKILL.md. Write <SENTIMENT_RUNS_DIR>\premarket-scores.txt with one line per headline:
...
STEP 3 — aggregate
  python ".claude\skills\finviz-market-sentiment\finviz_sentiment.py" aggregate --scores "<SENTIMENT_RUNS_DIR>\premarket-scores.txt" --state "<SENTIMENT_RUNS_DIR>\premarket-state.json" --out "<SENTIMENT_RUNS_DIR>\premarket-sentiment.json"

STEP 4 — record history, then render
  python ".claude\skills\finviz-market-sentiment\history.py" append --report "<SENTIMENT_RUNS_DIR>\premarket-sentiment.json" --label "Pre-market" --history "<SENTIMENT_RUNS_DIR>\history.json"
  ...
  python ".claude\skills\finviz-market-sentiment\render_artifact.py" --report "<SENTIMENT_RUNS_DIR>\premarket-sentiment.json" --label "Pre-market" --out "<SENTIMENT_RUNS_DIR>\dashboard.html"
  python ".claude\skills\finviz-market-sentiment\send_report.py" --report "<SENTIMENT_RUNS_DIR>\premarket-sentiment.json" --label "Pre-market" --out "<SENTIMENT_RUNS_DIR>\premarket-report.txt"
...
a) PushNotification (status "proactive"), ONE line under 200 chars, no markdown:
   "Pre-market: <SIGNAL> <index>/100 <conviction> — top 3x <TICKER>. <5-8 word driver>."
   If no recommendation was issued, say so instead of naming a ticker, e.g.:
   "Pre-market: SELL 41.2/100 moderate — no 3x rec (bearish tape). <driver>."
b) SendUserFile with <SENTIMENT_RUNS_DIR>\premarket-report.txt, status "proactive", display "attach", one-line caption with signal and index.

STEP 7 — final message
State signal, index, conviction, the picks (or that none were issued), whether a history row was recorded, and confirm both artifacts updated at their existing URLs.
